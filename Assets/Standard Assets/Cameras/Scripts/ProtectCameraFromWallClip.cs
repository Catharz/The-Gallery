...
namespace UnityStandardAssets.Cameras
{
	public class ProtectCameraFromWallClip : MonoBehaviour
	{
		...
		public string[] dontClipTags;
		// don't clip against objects with these tags (useful for not clipping against the targeted object)
		...
		private bool HasNoClipTag (Rigidbody rb)
		{
			for (int i = 0; i < dontClipTags.Length; i++) {
				if (dontClipTags [i] == rb.tag) {
					return true;
				}
			}
			return false;
		...
		private void LateUpdate ()
		{
			...
			for (int i = 0; i < cols.Length; i++) {
				if ((!cols [i].isTrigger) &&
				    !(cols [i].attachedRigidbody != null && HasNoClipTag (cols [i].attachedRigidbody))) {
			...
			for (int i = 0; i < m_Hits.Length; i++) {
				...
				if (m_Hits [i].distance < nearest && (!m_Hits [i].collider.isTrigger) &&
				    ...
				    HasNoClipTag (m_Hits [i].collider.attachedRigidbody))) {
